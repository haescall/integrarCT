...
class crud_fases
{
	...
	public function create($descripcion)
	{
		try
		{
			$stmt = $this->db->prepare("INSERT INTO fases(  descripcion,
                                                            created_at
                                      ) VALUES(             :descripcion,
                                                            CURDATE()
                                      )");
	...
	{
		$stmt = $this->db->prepare("SELECT * FROM fases WHERE id=:id");
	...
	public function update($id,$descripcion)
	{
		try
		{
			$stmt=$this->db->prepare("UPDATE fases SET descripcion= :descripcion,
													   updated_at=CURDATE()
													WHERE id=:id ");
			$stmt->bindparam(":descripcion",$descripcion);
	...
	{
		$stmt = $this->db->prepare("DELETE FROM fases WHERE id=:id");
		$stmt->bindparam(":id",$id);
		$stmt->execute();
		return true;
	}
	...
	public function dataview($query)
	{
		...
		if($stmt->rowCount()>0)
		{
			while($row=$stmt->fetch(PDO::FETCH_ASSOC))
			{
                ...
                <td align="center">
                <a href="edit-data_fases.php?edit_id=<?php print($row['id']); ?>"><i class="glyphicon glyphicon-edit"></i></a>
                </td>
                <td align="center">
                <a href="delete_fases.php?delete_id=<?php print($row['id']); ?>"><i class="glyphicon glyphicon-remove-circle"></i></a>
                </td>
                </tr>
                <?php
			}
